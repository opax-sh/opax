...
2. **SDK** — typed read/write access to spec-conformant data, hook event capture, plugin loading, storage lifecycle. Go, using go-git for core git object operations and modernc.org/sqlite for the query database. Single binary, no daemon or extra services beyond a standard Git environment.
...
- **Git is the host platform** — Opax keeps repo/object/state access in Go, but hook-time commit-message semantics may delegate to native Git instead of reimplementing trailer behavior.
